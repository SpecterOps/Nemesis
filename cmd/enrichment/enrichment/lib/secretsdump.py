...
class SAMHashes(OfflineRegistry):
    def __init__(self, samFile, bootKey = None, isRemote = False, perSecretCallback = lambda secret: _print_helper(secret)):
    ...
    def dumpEnc(self):
        ...
        for rid in rids:
            ...
            hashes.append((userName, rid, encLMHash.getData(), encNTHash.getData(), newStyle))
        return hashes
...
def decryptSAMHash(rid, cryptedHashBytes, hashedBootKey, is_nt):
    ...
    try:
        ...
        try:
            rc4Key = MD5( hashedBootKey[:0x10] + pack("<L",rid) + constant )
            rc4 = ARC4.new(rc4Key)
            key = rc4.encrypt(cryptedHash['Hash'])
        except:
            return None
...
class LSASecrets(OfflineRegistry):
    ...
    def __init__(self, securityFile, bootKey=None, remoteOps=None, isRemote=False, history=False,
