...
def build_about_expander():
    with st.expander("About Files"):
        st.markdown(
            """
        This page shows downloaded files processed by Nemesis, searchable by a variety of filters.

        The `Upload File` tab allows you to upload one or more files to Nemesis for processing.
"""
        )
...
def build_page(username: str):
    ...
    chosen_tab = stx.tab_bar(
        data=[
            stx.TabBarItemData(id=1, title="Downloaded Files", description="Downloaded Files Processed by Nemesis"),
            stx.TabBarItemData(id=2, title="Upload File", description="Manually Upload a File to Nemesis for Processing"),
        ],
        default="1",
    )

    if chosen_tab == str(1):  # downloaded_files
        build_file_triage_tab()
    elif chosen_tab == str(2):  # upload_file
        build_upload_file_tab()
