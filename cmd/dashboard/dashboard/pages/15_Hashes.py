...
def render_page(username: str):
    ...
    object_id = utils.get_single_valued_param("object_id") if utils.get_single_valued_param("object_id") else ""

    object_id = st.text_input("Originating object ID", value=object_id)
...
"""
    ...
    if object_id:
        if not utils.is_uuid(object_id):
            st.error(f"Object ID {object_id} is not a UUID!")
        else:
