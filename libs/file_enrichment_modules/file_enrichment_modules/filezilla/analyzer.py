...
rule Detect_FileZilla_Config {
    ...
    def should_process(self, object_id: str) -> bool:
        ...
        if not (filename_lower in ["sitemanager.xml", "recentservers.xml", "filezilla.xml"] or
                "filezilla" in filename_lower):
            return False
        ...
        logger.warning(f"FileZillaParser should_run: {should_run}")
        return should_run
    ...
    def _create_finding_summary(self, servers: List[Dict], file_name: str) -> str:
        ...
        for i, server in enumerate(servers, 1):
            ...
            if server.get('sync_browsing'):
                summary += f"* **Sync Browsing**: Enabled\n"
    ...
    def _has_credentials(self, servers: List[Dict]) -> bool:
        ...
        for server in servers:
            ...
            if (username and username != '<NO USERNAME>' and
                password and password not in ['<NO PASSWORD>', '<EMPTY PASSWORD>', '<PROTECTED BY MASTER PASSWORD>']):
                return True
        return False
