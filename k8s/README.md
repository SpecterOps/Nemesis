...
> **Looking for cloud deployment?** See the [EKS Deployment Guide](../docs/eks.md) for deploying Nemesis to AWS EKS.
...
EKS runs on AWS — suited for persistent cloud deployments with managed node groups and internet-accessible endpoints. See the [full EKS guide](../docs/eks.md) for detailed instructions.
...
# Access Nemesis at the NLB hostname printed by the setup script
# Credentials are generated by the setup script and displayed at the end
```
...
# Remove only Nemesis and Helm releases, keep EKS cluster running
./k8s/scripts/teardown-cluster-eks.sh --keep-cluster
```

> **Warning:** EKS incurs AWS charges (~$380/month). Always tear down when done.
