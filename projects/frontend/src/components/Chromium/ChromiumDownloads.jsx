...
const Row = React.memo(({ index, style, data }) => {
  ...
  return (
    ...
    >
      ...
      <div 
        className={`px-2 flex-shrink-0 w-64 text-sm text-gray-500 dark:text-gray-400 text-left cursor-pointer select-text transition-colors ${copiedCell === 'url' ? 'bg-green-200 dark:bg-green-800' : ''}`}
        onDoubleClick={() => handleCellDoubleClick(record.url, 'url')}
        title="Double-click to copy"
      >
        <Tooltip content={record.url || ''} side="top" align="start">
          <span className="block truncate">{record.url || ''}</span>
        </Tooltip>
      </div>
      <div 
        className={`px-2 flex-shrink-0 w-44 text-sm text-gray-500 dark:text-gray-400 text-left cursor-pointer select-text transition-colors ${copiedCell === 'end_time' ? 'bg-green-200 dark:bg-green-800' : ''}`}
        onDoubleClick={() => handleCellDoubleClick(record.end_time ? new Date(record.end_time).toLocaleString() : '', 'end_time')}
        title="Double-click to copy"
      >
        {record.end_time ? new Date(record.end_time).toLocaleString() : ''}
      </div>
      <div 
        className={`px-6 flex-grow text-sm text-gray-500 dark:text-gray-400 truncate text-left cursor-pointer select-text transition-colors ${copiedCell === 'download_path' ? 'bg-green-200 dark:bg-green-800' : ''}`}
        onDoubleClick={() => handleCellDoubleClick(record.download_path, 'download_path')}
        title="Double-click to copy"
      >
        <Tooltip content={record.download_path || ''} side="top" align="start" maxWidth="full">
          <span>{record.download_path || ''}</span>
        </Tooltip>
      </div>
    </div>
  );
...
const ChromiumDownloads = ({ renderActions }) => {
  ...
  const [downloadPathFilter, setDownloadPathFilter] = useState(() => searchParams.get('download_path') || '');
  const [sortColumn, setSortColumn] = useState(() => searchParams.get('sort_column') || 'end_time');
  ...
  const buildWhereClause = () => {
    ...
    if (downloadPathFilter) {
      conditions.push({ download_path: { _ilike: downloadPathFilter.replace(/\*/g, '%') } });
    }
  ...
  const buildOrderByClause = () => {
    ...
    switch (sortColumn) {
      ...
      case 'url':
        ...
        break;
      case 'download_path':
        orderBy.download_path = sortDirection;
        break;
      case 'end_time':
        orderBy.end_time = sortDirection;
        break;
      default:
        orderBy.end_time = sortDirection;
    }
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      const exportQuery = {
        query: `
          query GetDownloadsExport($where: chromium_downloads_bool_exp, $order_by: [chromium_downloads_order_by!]) {
            chromium_downloads(
              where: $where,
              order_by: $order_by
            ) {
      ...
      const data = result.data.chromium_downloads;
      ...
      const csvContent = [
        ...
        ...data.map(record => [
          ...
          `"${(record.download_path || '').replace(/"/g, '""')}"`,
          record.end_time ? `"${new Date(record.end_time).toISOString()}"` : '""'
        ].join(','))
      ].join('\n');
      ...
      link.setAttribute('download', `chromium_downloads_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  useEffect(() => {
    ...
    if (downloadPathFilter) {
      params.set('download_path', downloadPathFilter);
    }
  ...
  }, [sourceFilter, usernameFilter, browserFilter, urlFilter, downloadPathFilter, sortColumn, sortDirection, setSearchParams]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const countQuery = {
          query: `
            query GetDownloadsCount($where: chromium_downloads_bool_exp) {
              chromium_downloads_aggregate(where: $where) {
                aggregate {
                  count
                }
              }
        ...
        const count = countResult.data.chromium_downloads_aggregate.aggregate.count;
        setTotalCount(count);
        ...
        if (count > PAGINATION_THRESHOLD) {
          ...
          dataQuery = {
            query: `
              query GetDownloadsPaginated($where: chromium_downloads_bool_exp, $limit: Int!, $offset: Int!, $order_by: [chromium_downloads_order_by!]) {
                chromium_downloads(
                  where: $where,
                  limit: $limit,
                  offset: $offset,
          ...
          dataQuery = {
            query: `
              query GetDownloadsAll($where: chromium_downloads_bool_exp, $order_by: [chromium_downloads_order_by!]) {
                chromium_downloads(
                  where: $where,
                  order_by: $order_by
                ) {
        ...
        setRecords(dataResult.data.chromium_downloads);
      } catch (err) {
        console.error('Error fetching chromium downloads:', err);
    ...
    fetchData();
  }, [currentPage, sourceFilter, usernameFilter, browserFilter, urlFilter, downloadPathFilter, sortColumn, sortDirection]);
      ...
      <div className="p-2 border-b dark:border-gray-700 overflow-x-auto">
        <div className="flex items-center space-x-4 min-w-max">
          ...
          <div className="flex items-center space-x-2">
            ...
            <input
              type="text"
              placeholder="Filter by download path"
              className="border dark:border-gray-700 dark:bg-dark-secondary dark:text-gray-300 rounded p-2"
      ...
      {error && (
        <div className="p-2 bg-red-50 dark:bg-red-900/20 rounded-lg flex items-center space-x-2 m-4">
          ...
          <div className="flex flex-col">
            <span className="text-red-600 dark:text-red-400">Error loading chromium downloads: {error}</span>
      ...
      {!isLoading && !error && records.length === 0 && totalCount === 0 && (
        <div className="flex flex-col items-center justify-center p-12 text-center">
          <div className="text-gray-500 dark:text-gray-400 mb-6">
            ...
            <h3 className="text-lg font-medium mb-2">No chromium downloads found</h3>
            <p className="max-w-md mx-auto">
              No chromium download records match your current filters.
            </p>
          </div>
        </div>
      )}
...
export default ChromiumDownloads;