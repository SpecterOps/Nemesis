"""Text translation agent using Pydantic AI."""
...
class TextTranslator(BaseAgent):
    """Agent for translating text content using LLM."""
    ...
    def get_prompt(self) -> str:
        """Get the text translation prompt from database or use default."""
        try:
    ...
    def execute(self, ctx: WorkflowActivityContext, activity_input: dict) -> dict:
        """Translate text content in the given file to the specified target language."""
        object_id = activity_input.get("object_id", "")
        target_language = activity_input.get("target_language", "English")

        logger.debug("text_translation activity started", object_id=object_id, target_language=target_language)
        ...
        try:
            ...
            if not text_content:
                return {"success": False, "error": "No text content found to translate"}
            ...
            prompt = f"Please translate this text to {target_language}. If the document contains multiple languages, translate all text to {target_language}:\n\n{text_content}"

            result = agent.run_sync(prompt)
            logger.debug(
                "Text translation LLM analysis completed",
                object_id=object_id,
            ...
            # Store the translation as a file
            with tempfile.NamedTemporaryFile(mode="w", encoding="utf-8") as tmp_translation:
                tmp_translation.write(translated_text)
                tmp_translation.flush()
                translation_id = self.storage.upload_file(tmp_translation.name)

            # Add transform to database
            with psycopg.connect(self.postgres_connection_url) as conn:
                with conn.cursor() as cur:
                    metadata = {
                        "file_name": f"translation_{target_language.lower().replace(' ', '_')}.md",
                        "display_type_in_dashboard": "markdown",
                        "display_title": f"Translation ({target_language})",
            ...
            logger.debug("Text translation completed", object_id=object_id, target_language=target_language)
            return {"success": True, "transform_id": translation_id}

        except Exception as e:
            logger.error("Text translation failed", object_id=object_id, error=str(e))
            return {"success": False, "error": str(e)}


def translate_text(ctx: WorkflowActivityContext, activity_input: dict) -> dict:
