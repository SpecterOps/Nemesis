...
VALIDATORS = [
    ...
    Validator(
        "mythic.credential.username", must_exist=False, when=Validator("mythic.credential.token", must_exist=True)
    ),
    Validator(
        "mythic.credential.password", must_exist=False, when=Validator("mythic.credential.token", must_exist=True)
    ),
    # Nemesis validators
