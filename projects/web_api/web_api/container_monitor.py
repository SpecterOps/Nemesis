...
def find_config_file(start_path: Path, root_path: Path) -> Optional[Path]:
    """
...
def parse_config_file(config_path: Optional[Path], container_file: Path) -> Dict[str, Any]:
    """
        ...
        while self.running:
            try:
                ...
                try:
                    ...
                    result = self.processor.process_container_from_path(
                        container_id, file_path, processing_metadata
                    )
                    
                    logger.info(
                        f"Container processing completed: {file_path}",
    ...
    def start(self):
        ...
        try:
            ...
            self.processing_thread = threading.Thread(
                target=self._process_containers,
                daemon=True,
