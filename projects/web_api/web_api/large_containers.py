...
class ContainerProgress:
    ...
    def get_progress(self, container_id: str) -> Optional[Dict[str, Any]]:
...
class BaseContainerExtractor:
    ...
    def __init__(self, storage: StorageMinio, dapr_client: DaprClient, progress_tracker: ContainerProgress):
        ...
        self.filter_stats = {
            "files_processed": 0,
            "files_skipped_by_filter": 0,
            "files_skipped_by_error": 0
        }
    ...
    def set_container_info(self, container_id: str, file_metadata: Dict[str, Any]):
        ...
        if self.file_metadata.get("expiration") is None:
            expiration_dt = datetime.now(timezone.utc) + timedelta(days=DEFAULT_EXPIRATION_DAYS)
...
class FilePathFilter:
    ...
    def __init__(self, filters: Optional[FileFilters] = None):
...
class ZipContainerExtractor(BaseContainerExtractor):
    ...
    def extract_and_process(self, container_file_path: Path) -> int:
        ...
        logger.info(
            "Container extraction completed",
            container_id=self.container_id,
            stats=stats
        )
...
class LargeContainerProcessor:
    ...
    def create_container_record(self, container_id: str, container_type: str, file_metadata: Dict[str, Any], estimated_files: int, estimated_size: int) -> None:
        ...
        try:
            ...
            logger.info("Created container processing record", container_id=container_id, estimated_files=estimated_files)
        except Exception as e:
            logger.error(f"Error creating container record: {e}", container_id=container_id)
            raise

    def update_container_extraction_progress(self, container_id: str, total_files_extracted: int, total_bytes_extracted: int) -> None:
            ...
            with psycopg.connect(self.postgres_connection_string) as conn:
                with conn.cursor() as cur:
                    ...
                    """, (
                        ...
                        ContainerStatus.EXTRACTED,
                        container_id
                    ))
                    conn.commit()

            logger.debug("Updated container extraction progress",
                        container_id=container_id,
    ...
    def update_container_workflow_progress(self, container_id: str, file_size: int = 0, increment_completed: bool = False, increment_failed: bool = False) -> bool:
                    ...
                    if row:
                        ...
                        if all_complete:
                            cur.execute("""
                                ...
                                WHERE container_id = %s
                            """, (
                                ContainerStatus.WORKFLOWS_COMPLETE,
                                datetime.now(),
                                container_id
                            ))
    ...
    def detect_container_type(self, filename: str, container_file_path: Path) -> Optional[str]:
        ...
        disk_image_extensions = (
            # Raw disk images
            '.dd', '.raw', '.img', '.image', '.bin', '.dmg',
            # Forensic formats
            '.e01', '.ex01', '.l01', '.lx01', '.ewf', '.s01'
        )
    ...
    def process_container_from_path(self, container_id: str, file_path: Path, metadata: dict) -> Dict[str, Any]:
        ...
        try:
            ...
            container_type = self.detect_container_type(
                metadata.get("filename", file_path.name),
                file_path
            )
            ...
            if not container_type:
                raise HTTPException(
                    status_code=400,
                    detail="Unsupported container type"
                )
